 ...
 * default style, laid out like the elements around it. The span of a removed element is not written, so the text around
 * it is adjusted: a comma is written where two elements no longer have one between them, and the comma after the last
 * element is dropped unless the document wrote one there.
 *
 * <p>
 ...
 * comment cannot be written for that version and throws {@link IllegalArgumentException}.
 */
final class EditedContainerSerializer {
   ...
   */
  enum Context {
    /** The value of a {@code key = value} line. A multi-line string is written only in this context. */
    LINE,
    /**
     * An element of an array, or the value of an entry of an inline table laid out over lines when the version allows
     * line breaks inside an inline table.
     */
    ELEMENT,
    /**
     * The value of an entry of an inline table written on one line, or written for TOML 1.0.0, which does not allow
     * line breaks inside an inline table: the value is written on the one line. An inline table in which a value holds
     * a comment is written over lines, so such a value is not written in this context.
     */
    ENTRY
  }
  ...
  // Whether the container keeps the layout the document gave it, so that the text between its elements is copied with
  // them
  ...
  // Whether a comma follows the last element of the container, which is the case when the document wrote a comma after
  // its last element
   ...
   * @throws IllegalArgumentException If a comment has to be written in an inline table and the version written is TOML
   *         1.0.0.
   */
  static void append(
      StringBuilder text,
  ...
  private void write(ElementContainer<?> container, Context context) throws IOException {
    ...
    for (int i = 0; i < items.size(); i++) {
      ...
      if (item.isEntry() && commaOwed && (leading == null || !holdsComma(leading))) {
        // The comma is written directly after the value it follows
   ...
   *
   * <p>
   * A dotted key writes one entry, so a table with no entries cannot be written as dotted keys and is written as an
   * empty inline table instead; and a comment written among those keys would be read in the inline table rather than in
   * this one, which is also why a table whose own entry carries a comment is written as a value of its own.
   *
   ...
   *
   * <p>
   * The container is written over lines if the document wrote it over lines, or if an item needs a line of its own. An
   * inline table holding a comment is written over lines, so it cannot be written for TOML 1.0.0, which allows no line
   * break inside an inline table; one the document wrote over lines with no comment in it is copied as it was.
   *
   * @param items The items, in the order they are written.
   * @param table Whether the container is an inline table.
   * @throws IllegalArgumentException If the container is an inline table holding a comment and the version written is
   *         TOML 1.0.0.
   */
  private void layout(List<Item> items, boolean table) {
    ...
    if (table && ownLines) {
      Serializer.requireInlineTableOverLines(options.version());
    }
  ...
  private static boolean needsOwnLine(Item item) {
    ...
    return !entry.comments().isEmpty() || Serializer.holdsComments(entry.value.get());
  }
  ...
  private void appendTail(Item item, boolean entryFollows) {
    ...
    if (entry.commentsModified()) {
      // The comma the document wrote is in the tail with the comment, which is not copied, so a comma is written where
      // it is needed: before the comment, or by the text of the next item, which may already hold one
      boolean written = appendCommentAfter(entry, afterSpacing(span));
      String newline = newlineOf(span);
      // A comment runs to the end of its line, so what the document wrote after the entry on that line is written on
      // the next line
  ...
  private void appendCommentAbove(Entry entry) {
    for (TomlComment comment : entry.comments()) {
      ...
      for (String rawLine : comment.rawLines()) {
        // A run is attached ABOVE an entry only when each of its lines is on a line by itself
        startElementLine();
  ...
  private void appendKey(List<String> keyPath) {
    ...
    Serializer.appendKeyPath(out, keyPath);
    out.append(" = ");
  }

  /**
   * Write a value anew at the end of the line the output has reached: the literal it was read as, where it was read
   * from a line of this document or another, and otherwise the value in the default style.
   *
  ...
  private static final class Item {
    ...
    /** Where the document wrote this item, or {@code null} if it has no usable span. */
    @Nullable
