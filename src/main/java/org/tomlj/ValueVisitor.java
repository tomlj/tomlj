...
final class ValueVisitor extends TomlParserBaseVisitor<Object> {
  ...
  /**
   * Add the comments a line break holds that document nothing to the array or inline table they were written in.
   *
   ...
   * @return The record, or {@code null} if no source was kept, or if the parser conjured a bracket the document does
   *         not hold.
   */
  @Nullable
   ...
   * first offset not yet taken by one. Each element takes the text from the cursor - its leading whitespace, and the
   * comment run above it - through its key and value to the end of its tail: the comma that follows it, the comment
  ...
  private static final class ElementSpans {
    ...
    SourceSpan element(
      ...
      int keyStop = -1;
      if (keyContext != null) {
        keyStart = keyContext.getStart().getStartIndex();
        keyStop = keyContext.getStop().getStopIndex();
      }
      int afterStart = -1;
      int afterStop = -1;
      TerminalNode after = Comments.commentAfter(nodes, index);
      if (after != null) {
        afterStart = after.getSymbol().getStartIndex();
        afterStop = after.getSymbol().getStopIndex();
      }
      ...
      if (isToken(next, TomlParser.Comma)) {
        ...
        commaOffset = comma.getStartIndex();
        stop = comma.getStopIndex();
        next = at(nodes, index + 2);
      }
      if (next instanceof TomlParser.LineBreakContext) {
        // The line break's first newline ends the element's line, and takes the comment after the element with it. The
        // newlines and runs written after it are lines of their own, and belong to whatever follows.
      ...
      return SourceSpan
          .element(
              ...
              keyStop,
              keyParts,
              valContext.getStart().getStartIndex(),
              commaOffset,
              afterStart,
              afterStop,
              valueStop + 1,
    ...
    /**
     * Record where a comment that documents nothing was written, taking its text from the cursor to the newline ending
     * its last line.
     *
     * @param node The comment, as {@link Comments#unattached} gives it back.
    ...
    SourceSpan comment(ParseTree node, TomlParser.LineBreakContext lineBreak) {
      ...
      if (node instanceof TerminalNode) {
        ...
        // A comment ending a line takes that line's newline, which the line break holds directly after it.
      ...
      return SourceSpan.comment(source, start, commentStart, commentStop);
    }
