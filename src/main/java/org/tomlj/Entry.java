...
abstract class Entry implements MutableTomlEntry {
  ...
  public MutableTomlEntry setComment(String text, TomlComment.Placement placement) {
    ...
    List<String> lines = (placement == TomlComment.Placement.ABOVE) ? Arrays.asList(text.split("\n", -1))
        : Collections.singletonList(text);
    return updateAttachedComment(placement, TomlComment.ofLines(lines, placement));
  }
  ...
  private MutableTomlEntry updateAttachedComment(TomlComment.Placement placement, @Nullable TomlComment comment) {
    TomlComment above =
        (placement == TomlComment.Placement.ABOVE) ? comment : attachedComment(TomlComment.Placement.ABOVE);
    TomlComment after =
        (placement == TomlComment.Placement.AFTER) ? comment : attachedComment(TomlComment.Placement.AFTER);
